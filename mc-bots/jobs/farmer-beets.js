...
async function run(state, task, options) {
  ...
  while (!task.cancelled) {
    ...
    if (ripe) {
      bot.chat(`Found ripe beetroots at ${ripe.position.x}, ${ripe.position.y}, ${ripe.position.z}!`)
      try {
        await bot.dig(ripe)
        notifyHarvest(bot, 1)
    ...
    if (plot) {
      searchCount = 0
      bot.chat(`Found empty farmland, planting beetroot seeds...`)
      try {
        ...
        bot.chat(`Planted beetroot seeds! Remaining: ${seedItem.count - 1}`)
        lastActivity = Date.now()
      } catch (error) {
